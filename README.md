...
- Results can be filtered locally using Booking.com's car categories (Small, Medium, Large, Estate, Premium, People carriers, or SUVs), pickup-location types (Airport terminal or shuttle, City centre, Train station, Shuttle bus, or In terminal), and supplier review score (7+, 8+, or 9+) without making another provider request. Choosing an airport suggestion automatically restricts results to in-terminal and shuttle-bus depots; choosing a suggestion labelled City Centre/Center selects the City centre depot filter.
- Vehicle categories prioritise the provider's specific `group_or_similar` description, so an offer described as an intermediate SUV remains in the SUV filter rather than being treated as a generic medium car.
